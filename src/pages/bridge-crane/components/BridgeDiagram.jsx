...
const BridgeDiagram = ({ specs, validationErrors, isMobile = false }) => {
  const hasErrors = Object.keys(validationErrors)?.length > 0;
  
  return (
    <div className="space-y-6">
      {/* Bridge Crane Diagram */}
      <div className="bg-surface border border-border rounded-lg p-6">
        <div className="flex items-center space-x-2 mb-4">
          <Icon name="Image" size={20} className="text-primary" />
          <h3 className="text-lg font-semibold text-text-primary">Bridge Crane Diagram</h3>
        </div>
        ...
        <div className="bg-secondary-50 border border-secondary-200 rounded-lg p-4 mb-4 font-mono text-xs">
          <div className="text-center">
            ...
            <div className="mb-2">│           BRIDGE CRANE             │</div>
            <div className="mb-2">├─────────────────────────────────────┤</div>
            <div className="mb-2">│  ┌─────┐    ┌─────┐    ┌─────┐    │</div>
            <div className="mb-2">│  │ Trolley │    │ Hook  │    │ Load  │    │</div>
            <div className="mb-2">│  └─────┘    └─────┘    └─────┘    │</div>
            <div className="mb-2">├─────────────────────────────────────┤</div>
            <div className="mb-2">│           Bridge Span              │</div>
            <div className="mb-2">└─────────────────────────────────────┘</div>
            <div className="mt-2">        ↑</div>
            <div className="mt-1">    Runway Rails</div>
          </div>
        </div>
      ...
      <div className="bg-surface border border-border rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-text-secondary">Bridge Speed:</span>
              <span className="font-medium">{specs?.basic?.bridgeSpeed || 0} m/min</span>
            </div>
            <div className="flex justify-between">
      ...
      {isMobile && (
        <div className="bg-info-50 border border-info-200 rounded-lg p-4">
          ...
          <p className="text-sm text-info-700">
            Use the specifications panel above to configure your bridge crane parameters.
          </p>
        </div>
      )}
    </div>
